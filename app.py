...
def index():
    return render_template(
        "index.html",
        projects=[],
        message="Silakan upload file Excel untuk memulai"
    )
...
def detail(kode):
    ...
    if df_global is None or kolom_kode is None:
        return "Data belum diupload. Silakan upload ulang."

    data = df_global[df_global[kolom_kode].astype(str) == kode]
    if data.empty:
        return f"Data dengan kode {kode} tidak ditemukan"
    ...
    return render_template(
        "detail.html",
        kode=kode,
        table=data.apply(lambda col: col.map(format_nominal)).to_html(index=False),
